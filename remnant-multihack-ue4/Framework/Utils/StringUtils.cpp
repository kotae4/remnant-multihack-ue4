...
void Utils::Strings::PrintErrorMessage(HRESULT error)
{
	...
	if (errorText != NULL)
	{
		...
		LOG_ERRORW(L"Error code (0x{}): {}", error, errorText);
...
std::string Utils::Strings::GetErrorMessage(HRESULT error)
{
	LPTSTR errorText = NULL;
...
VOID Utils::Strings::PrintLastError()
{
	...
	{
		// ... do something with the string `errorText` - log it, display it to the user, etc.
		LOG_ERRORW(L"Error code (0x{}): {}", errCode, errorText);
		// release memory allocated by FormatMessage()
		// < win10
		//LocalFree(errorText);
		// win10
...
BOOL Utils::Strings::TryConvertUtf8ToUtf16(const std::string& utf8, OUT std::wstring& outWStr)
{
	if (utf8.empty())
	{
		return FALSE;
...
BOOL Utils::Strings::TryConvertUtf16ToUtf8(const std::wstring& utf16, OUT std::string& outStr)
{
	if (utf16.empty())
	{
		return FALSE;
...
std::vector<std::string> Utils::Strings::SplitA(const std::string& input, const std::string& delim)
{
...
std::vector<std::wstring> Utils::Strings::SplitW(const std::wstring& input, const std::wstring& delim)
{
